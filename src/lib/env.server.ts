 ...
 * Проверка ниже — страховка на время выполнения. Когда в проекте появится
 * пакет server-only, её место займёт ошибка на этапе сборки, которая
 * ловит нарушение раньше и надёжнее.
 *
...
// Импорты в модулях ES поднимаются наверх, поэтому проверка стоит после них,
// а не до: расположить её выше было бы самообманом — она всё равно
// выполнится после разбора импортов.
if (typeof window !== 'undefined') {
  throw new Error(
    'src/lib/env.server.ts импортирован в браузерном коде. ' +
      'Серверные переменные не должны попадать в клиентский бандл.',
  );
}
